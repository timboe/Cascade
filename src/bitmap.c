...
LCDBitmap* m_rectBitmap[MAX_PEG_SIZE][128];
...
LCDBitmap* getBitmapPeg(const struct Peg_t* p) {
  switch (p->m_shape) {
    ...
    case kPegShapeRect: return m_rectBitmap[p->m_size][p->m_iAngle % 128]; // Symmetry
    default: return NULL;
  }
  return NULL;
}
...
void initBitmap() {
  ...
  for (int i = 0; i < 9;  ++i) {
    pd->graphics->fillRect(buf + i*3*BALL_RADIUS, buf + (12 - par[i])*2*BALL_RADIUS, 3*BALL_RADIUS, par[i]*2*BALL_RADIUS, kColorWhite);
  }
  ...
  for (int i = 0; i < 9; ++i) {
    for (int j = 1; j <= balls[i]; j++) {
      pd->graphics->drawBitmap(m_ballBitmap[0], buf + BALL_RADIUS/2 + i*3*BALL_RADIUS, buf + (12 - j)*2*BALL_RADIUS, kBitmapUnflipped);
    }
  }

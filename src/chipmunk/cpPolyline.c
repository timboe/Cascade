...
ApproximateConcaveDecomposition(cpVect *verts, int count, cpFloat tol, cpPolylineSet *set)
{
	...
	int hullCount = cpConvexHull(count, verts, hullVerts, &first, 0.0f);
...
cpPolylineConvexDecomposition_BETA(cpPolyline *line, cpFloat tol)
{
	...
	cpAssertSoft(cpAreaForPoly(line->count, line->verts, 0.0f) >= 0.0f, "Winding is backwards. (Are you passing a hole?)");
